...
using CashSwiftUtil.Reporting.FailedPostsReporting;
using OfficeOpenXml;
    ...
    public class ExcelManager
    {
        ...
        public static byte[] GenerateCITExcelAttachment(CITReport CITReport, string path)
        {
            using (ExcelPackage excel = new ExcelPackage())
            {
                ...
                foreach (IGrouping<string, CITDenomination> source in (IEnumerable<IGrouping<string, CITDenomination>>)CITReport.CITDenominations.GroupBy(x => x.Currency).OrderBy(y => y.Key))
                {
                    ...
                    ExcelTable excelTable = excelWorksheet3.Tables.First();
          ...
          string path)
        {
            ...
            using (ExcelPackage excel = LoadExcelFromPath(templatePath))
            {
                ...
                excel.Workbook.Worksheets.First().Select();
                return SaveExcelToPath(path, excel);
            }
        }
    }
