...
// Type: CashSwiftUtil.Security.Authentication.LDAP.LDAPAuthentication
...
namespace CashSwiftUtil.Security.Authentication.LDAP
{
    public class LDAPAuthentication
    {
        ...
        public LdapConnection SetupConnection(NetworkCredential networkCredential)
        {
            try
            {
                ...
                if (IgnoreCert)
                    ldapConnection.SessionOptions.VerifyServerCertificate += (_param1, _param2) => true;
          ...
          string[] attributesToReturn = null)
        {
            ...
            {
                SearchRequest request = new SearchRequest(Domain, searchFilter, SearchScope.Subtree, null);
                return ldapConnection.SendRequest(request) as SearchResponse;
            }
        }
