...
{
    public class AlertTransactionEndedFailed : AlertBase
    {
        public const int ALERT_ID = 4007;
        private AppTransaction _transaction;

        public AlertTransactionEndedFailed(
          ...
          : base(device, dateDetected)
        {
            ...
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 4007);
        }
