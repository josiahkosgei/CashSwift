// Utils.AlertClasses.AlertHDDFull
...
{
    public class AlertHDDFull : AlertBase
    {
        ...
        {
            _errorMessage = string.Format("Setting CashSwiftDeviceState.HDD_FULL as HDD space of {0:0.##} GB < {1:0.##} GB", availableSpace, minimumSpace);
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 1102);
        }
