...
{
    internal class AlertBagRemoved : AlertBase
    {
        public const int ALERT_ID = 1210;
        private bool _duringCIT;

        public AlertBagRemoved(Device device, DateTime dateDetected, bool duringCIT = false)
          : base(device, dateDetected)
        {
            _duringCIT = duringCIT;
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 1210);
        }
        ...
        protected new string GenerateHTMLMessageToken() => "Bag Removed" + (_duringCIT ? " during CIT" : " outside normal operation");

        protected new string GenerateRawTextMessageToken() => "Bag Removed" + (_duringCIT ? " during CIT" : " outside normal operation");

        protected new string GenerateSMSMessageToken() => "Bag Removed" + (_duringCIT ? " during CIT" : " outside normal operation");
