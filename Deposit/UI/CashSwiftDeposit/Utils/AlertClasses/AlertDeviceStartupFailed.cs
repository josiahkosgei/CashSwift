...
{
    public class AlertDeviceStartupFailed : AlertBase
    {
        public const int ALERT_ID = 1101;
        private string _errorMessage;

        public AlertDeviceStartupFailed(string errorMessage, Device device, DateTime dateDetected)
          : base(device, dateDetected)
        {
            _errorMessage = errorMessage;
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 1101);
        }
