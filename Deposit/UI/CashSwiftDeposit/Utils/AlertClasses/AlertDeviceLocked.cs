...
{
    public class AlertDeviceLocked : AlertBase
    {
        public const int ALERT_ID = 1400;
        private string _errorMessage;

        public AlertDeviceLocked(string errorMessage, Device device, DateTime dateDetected)
          : base(device, dateDetected)
        {
            _errorMessage = errorMessage;
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 1400);
        }
