...
{
    public class AlertTransactionCancelled : AlertBase
    {
        public const int ALERT_ID = 4005;
        private AppTransaction _transaction;

        public AlertTransactionCancelled(
          DepositorLogger log,
          ...
          : base(device, dateDetected)
        {
            ...
            {
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 4005) ?? throw new NullReferenceException(string.Format("{0}:AlertID {1} is not in DB", nameof(AlertTransactionCancelled), 4005));
                if (transaction == null)
                    log.Warning(GetType().Name, "ArgumentNullException", "ArgumentNullException", "Variable transaction cannot be null");
                else
