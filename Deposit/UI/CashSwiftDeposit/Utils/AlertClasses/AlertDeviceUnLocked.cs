...
namespace CashSwiftDeposit.Utils.AlertClasses
{
    public class AlertDeviceUnLocked : AlertBase
    {
        ...
        public AlertDeviceUnLocked(string errorMessage, Device device, DateTime dateDetected)
          : base(device, dateDetected)
        {
            _errorMessage = errorMessage;
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
                AlertType = depositorDbContext.AlertMessageTypes.FirstOrDefault(x => x.id == 1401);
        }
