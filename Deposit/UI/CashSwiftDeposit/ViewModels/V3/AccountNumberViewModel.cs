...
namespace CashSwiftDeposit.ViewModels.V3
{
    ...
    public class AccountNumberViewModel : AccountNumberBase
    {
          ...
          : base(screenTitle, applicationViewModel, required)
        {
            ...
            AccountDetailsTitleCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_AccountDetailsTitleCaption", "Account Details");
            AccountNameLabelCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_AccountNameLabelCaption", "Account Name");
            ValidateButtonCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_AccountNumberValidateButtonCaption", "Confirm");
            NextCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_AccountNumberValidateButtonCaption", "Confirm");
            ChangeInputButtonCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_ChangeAccountNumberButtonCaption", "Change Account");
            AccountNumberLabelCaption = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel._ctor", "sys_AccountNumberLabelCaption", "Account Number");
            CanNext = false;
            ...
            try
            {
                ...
                if (num1 != 0 && ClientValidation(CustomerInput))
                {
                    if (Task.Run(() => ValidateAsync(CustomerInput)).Result)
                    {
                        ...
                        flag = true;
                        ApplicationViewModel.Log.Info(nameof(AccountNumberViewModel), "SUCCESS", "Validate", $"Account Number '{CustomerInput}' = Account Name '{AccountName}'");
                    }
                    else
                    {
                        int validationTries = ApplicationViewModel.CurrentTransaction.ValidationTries;
                        ++ApplicationViewModel.CurrentTransaction.ValidationTries;
                        ApplicationViewModel.Log.Warning(nameof(AccountNumberViewModel), "FAILED", "Validate", $"Validation of Account Number '{CustomerInput}' Failed. Incrementing attempts from {validationTries} to {ApplicationViewModel.CurrentTransaction.ValidationTries}");
                ...
                if (num3 != 0)
                {
                    ...
                    ErrorText = ApplicationViewModel.CashSwiftTranslationService.TranslateSystemText("AccountNumberViewModel.Validate", "sys_ValidateAccountNumberCallsExceededErrorMessage", "Too many validation requests. Kindly restart the transaction.");
                }
