...
namespace CashSwiftDeposit.Models
{
    public class DeviceConfiguration
    {
        ...
        public UtilDepositorLogger Log { get; set; } = new UtilDepositorLogger(Assembly.GetExecutingAssembly().GetName().Version, "CashSwiftDepositLog");
        ...
        public bool USE_REAR_SCREEN => _USE_REAR_SCREEN;
        ...
        {
            get => _SYSTEM_CODE;
            internal set => _SYSTEM_CODE = value;
        }
        public string BANK_ID
        {
            get => _BANK_ID;
            internal set => _BANK_ID = value;
        }
        public bool START_COUNT_AUTOMATICALLY { get; internal set; } = true;
        ...
        public static DeviceConfiguration GetInstance() => deviceConfiguration ?? new DeviceConfiguration();
          ...
          IList<(string config_id, string config_value)> config)
        {
            ...
            InitialiseConfig(config, "RECEIPT_FOLDER", "RECEIPT_FOLDER", ref _RECEIPT_FOLDER, "C:\\CashSwiftDeposit\\Receipts");
            ...
            InitialiseConfig(config, "USE_REAR_SCREEN", "USE_REAR_SCREEN", ref _USE_REAR_SCREEN, false);
            ...
            InitialiseConfig(config, "EMAIL_LOCAL_FOLDER", "EMAIL_LOCAL_FOLDER", ref _EMAIL_LOCAL_FOLDER, "C:\\CashSwiftDeposit\\EMAIL");
            ...
            InitialiseConfig(config, "TRANSACTION_LOG_FOLDER", "TRANSACTION_LOG_FOLDER", ref _TRANSACTION_LOG_FOLDER, "C:\\CashSwiftDeposit\\Transactions");
            ...
            InitialiseConfig(config, "SYSTEM_CODE", "SYSTEM_CODE", ref _SYSTEM_CODE, null);
            InitialiseConfig(config, "BANK_ID", "BANK_ID", ref _BANK_ID, null);
        }
        ...
        private IList<(string config_id, string config_value)> GenerateConfigs()
        {
            using (DepositorDBContext depositorDbContext = new DepositorDBContext())
            {
                ...
                var config_group = depositorDbContext.Devices.FirstOrDefault().config_group;
                var DeviceConfigs = depositorDbContext.DeviceConfigs.Where(x => x.group_id == config_group).ToList();
                Log.InfoFormat(nameof(DeviceConfiguration), "DeviceConfiguration", nameof(GenerateConfigs), "Setting Config {0}", config_group);

                DeviceConfigs.ForEach(deviceConfig => configs.First(x => x.name.Equals(deviceConfig.config_id, StringComparison.OrdinalIgnoreCase)).default_value = deviceConfig.config_value);
                return configs.Select<Config, (string, string)>(x => (x.name, x.default_value)).ToList();
            }
        }
