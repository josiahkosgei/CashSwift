...
namespace CashAccSysDeviceManager
{
    public class CashAccSysDeviceManager : DeviceManagerBase
    {
            ...
            NoteCount = new List<NoteCount>()
      {
        ...
        protected new CashSwiftLogger Log { get; set; }
        ...
        {
            Log = new CashSwiftLogger(Assembly.GetAssembly(typeof(DeviceMessageBase)).GetName().Version.ToString(), "DeviceManagerLog", null);
        ...
        public new void OnTransactionStartedEvent(object sender, DeviceTransaction e)
        {
         ...
         CurrentState
           ...
           CurrentState
                ...
                base.OnTransactionStartedEvent(this, e);
            }
            else
            {
        ...
        protected void OnStatusReportEvent(object sender, StatusReport statusReport)
        {
             ...
             CurrentState
                ...
                if (CurrentDeviceStatus.Bag.PercentFull != deviceStatusResult.ControllerStatus.Bag.PercentFull)
                {
                    if (deviceStatusResult.ControllerStatus.Bag.PercentFull >= 100)
                        OnBagFullAlertEvent(this, deviceStatusResult.ControllerStatus);
                    else if (deviceStatusResult.ControllerStatus.Bag.PercentFull >= BAGFULL_WARN_PERCENT)
                        OnBagFullWarningEvent(this, deviceStatusResult.ControllerStatus);
                }
                ...
                if (CurrentDeviceStatus.ControllerState != deviceStatusResult.ControllerStatus.ControllerState)
                {
                 ...
                 deviceStatusResult.ControllerStatus.ControllerState
                            });
                            if (deviceStatusResult.ControllerStatus.Transaction.Status != 0 && CurrentTransaction == null && DeviceManagerMode == DeviceManager.DeviceManagerMode.NONE)
                            {
                ...
                if ((currentDeviceStatus1 != null ? (currentDeviceStatus1.ControllerState == ControllerState.IDLE ? 1 : 0) : 0) != 0)
                {
                    if (HasEscrow)
                    {
                        ...
                        else
                        {
                            ...
                            num2 = status1.GetValueOrDefault() == escrowStatus & status1.HasValue ? 1 : 0;
                        }
                        if (num2 == 0)
                            goto label_49;
                    }
            ...
            label_50:
             ...
             deviceStatusResult.ControllerStatus.Transaction.Status
                    });
                    ...
                    if (CurrentTransaction == null)
                    {
                        ...
                        if ((currentDeviceStatus4 != null ? (currentDeviceStatus4.ControllerState == ControllerState.IDLE ? 1 : 0) : 0) != 0)
                        {
                            ...
                            else
                            {
                                ...
                                num4 = status3.GetValueOrDefault() == deviceState & status3.HasValue ? 1 : 0;
                            }
                            if (num4 != 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    EscrowStatus escrowStatus = EscrowStatus.IDLE_POS;
                                    num5 = status1.GetValueOrDefault() == escrowStatus & status1.HasValue ? 1 : 0;
                                }
                                if (num5 != 0)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        num3 = status2.GetValueOrDefault() == transactionStatus & status2.HasValue ? 1 : 0;
                                        goto label_77;
                                    }
                                }
                            }
               ...
               CurrentState
                ...
                if (deviceStatusResult.ControllerStatus.NoteAcceptor.Status == DeviceState.JAM && DeviceManagerMode != DeviceManager.DeviceManagerMode.NOTEJAM)
                    OnNoteJamStartEvent(this, EventArgs.Empty);
                else if (deviceStatusResult.ControllerStatus.NoteAcceptor.Status != DeviceState.JAM && DeviceManagerMode == DeviceManager.DeviceManagerMode.NOTEJAM && CurrentState == DeviceManagerState.NOTEJAM_START)
                    OnNoteJamEndEvent(this, EventArgs.Empty);
                CurrentDeviceStatus = deviceStatusResult.ControllerStatus;
                deviceStatusResult.DeviceManagerState = CurrentState;
                if (CurrentState == DeviceManagerState.OUT_OF_ORDER)
                    ResetDevice(false);
                NotifyCurrentTransactionStatusChanged();
                OnStatusReportEvent(this, deviceStatusResult);
            }
        }

        public override void Initialise() => CurrentTransaction = null;
          ...
          TransactionStatusResponse TransactionStatusResponse)
        {
            if (CurrentTransaction != null)
            {
             ...
             CurrentState
                    ...
                    {
                        Log.Error(nameof(CashAccSysDeviceManager), "ERROR", nameof(OnTransactionStatusEvent), "Invalid TransactionStatusResult: " + JsonConvert.SerializeObject(statusResponseResult, Formatting.Indented, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore }), Array.Empty<object>());
                        CurrentState = DeviceManagerState.OUT_OF_ORDER;
                        ResetDevice(false);
                    }
                }
                ...
                deviceTransactionResult.level = ErrorLevel.SUCCESS;
                deviceTransactionResult.data = CurrentTransaction;
                OnTransactionStatusEvent(this, deviceTransactionResult);
            }
            else
        ...
        public override void OnTransactionEndEvent(object sender, DeviceTransactionResult e)
        {
         ...
         CurrentState
           ...
           CurrentState
                ...
                CurrentState = DeviceManagerState.NONE;
                base.OnTransactionEndEvent(this, e);
            }
            else
            {
                if (CurrentState == DeviceManagerState.OUT_OF_ORDER)
        ...
        public override void OnCashInStartedEvent(object sender, DeviceTransactionResult e)
        {
         ...
         CurrentState
            ...
            base.OnCashInStartedEvent(this, e);
        }
        ...
        public override void OnCountStartedEvent(object sender, DeviceTransactionResult e)
        {
         ...
         CurrentState
            ...
            base.OnCountStartedEvent(this, e);
        }
        ...
        public override void OnCountPauseEvent(object sender, DeviceTransactionResult e)
        {
         ...
         CurrentState
           ...
           CurrentState
                ...
                base.OnCountPauseEvent(this, e);
        }

        public override void OnNoteJamStartEvent(object sender, EventArgs e)
        {
            CurrentState = DeviceManagerState.NOTEJAM_START;
            DeviceManagerMode = DeviceManager.DeviceManagerMode.NOTEJAM;
            base.OnNoteJamStartEvent(this, e);
        }
        ...
        public override void OnNoteJamClearWaitEvent(object sender, EventArgs e)
        {
            ...
            DeviceManagerMode = DeviceManager.DeviceManagerMode.NOTEJAM;
            base.OnNoteJamClearWaitEvent(this, e);
        }

        public override void OnNoteJamEndRequestEvent(object sender, EventArgs e)
        {
            if (DeviceManagerMode != DeviceManager.DeviceManagerMode.NOTEJAM || CurrentState == DeviceManagerState.NOTEJAM_END_REQUEST)
                return;
            CurrentState = DeviceManagerState.NOTEJAM_END_REQUEST;
            DeviceManagerMode = DeviceManager.DeviceManagerMode.NOTEJAM;
            base.OnNoteJamEndRequestEvent(this, e);
        }

        public override void OnNoteJamEndEvent(object sender, EventArgs e)
        {
            CurrentState = DeviceManagerState.NONE;
            DeviceManagerMode = DeviceManager.DeviceManagerMode.NONE;
            base.OnNoteJamEndEvent(this, e);
        }
        ...
        public override void OnEscrowRejectEvent(object sender, DeviceTransactionResult e)
        {
         ...
         CurrentState
           ...
           CurrentState
                ...
                base.OnEscrowRejectEvent(this, e);
        }
           ...
           CurrentState
            ...
            else
            {
             ...
             CurrentState
                    ...
                    base.OnEscrowOperationCompleteEvent(this, e);
                }
        ...
        public override void OnEscrowJamStartEvent(object sender, EventArgs e)
        {
            ...
            DeviceManagerMode = DeviceManager.DeviceManagerMode.ESCROW_JAM;
            base.OnEscrowJamStartEvent(this, e);
        }
        ...
        public override void OnEscrowJamClearWaitEvent(object sender, EventArgs e)
        {
            ...
            DeviceManagerMode = DeviceManager.DeviceManagerMode.ESCROW_JAM;
            base.OnEscrowJamClearWaitEvent(this, e);
        }

        public override void OnEscrowJamEndRequestEvent(object sender, EventArgs e)
        {
            if (DeviceManagerMode != DeviceManager.DeviceManagerMode.ESCROW_JAM || CurrentState == DeviceManagerState.ESCROWJAM_END_REQUEST)
                return;
            CurrentState = DeviceManagerState.ESCROWJAM_END_REQUEST;
            DeviceManagerMode = DeviceManager.DeviceManagerMode.ESCROW_JAM;
            base.OnEscrowJamEndRequestEvent(this, e);
        }
        ...
        public override void OnEscrowJamEndEvent(object sender, EventArgs e)
        {
            CurrentState = DeviceManagerState.NONE;
            DeviceManagerMode = DeviceManager.DeviceManagerMode.NONE;
            base.OnEscrowJamEndEvent(this, e);
        }
                    ...
                    if (CurrentTransaction == null && CurrentState == DeviceManagerState.NONE)
                    {
                        ...
                        if ((currentDeviceStatus1 != null ? (currentDeviceStatus1.ControllerState == ControllerState.IDLE ? 1 : 0) : 0) != 0)
                        {
                            ...
                            else
                            {
                                ...
                                DeviceState deviceState = DeviceState.IDLE;
                                num2 = status.GetValueOrDefault() == deviceState & status.HasValue ? 1 : 0;
                            }
                            if (num2 != 0)
                            {
                                IControllerStatus currentDeviceStatus3 = CurrentDeviceStatus;
                                ...
                                else
                                {
                                    ...
                                    EscrowStatus escrowStatus = EscrowStatus.IDLE_POS;
                                    num3 = status.GetValueOrDefault() == escrowStatus & status.HasValue ? 1 : 0;
                                }
                                if (num3 != 0)
                                {
                                    IControllerStatus currentDeviceStatus4 = CurrentDeviceStatus;
                                    ...
                                    else
                                    {
                                        ...
                                        DeviceTransactionStatus transactionStatus = DeviceTransactionStatus.NONE;
                                        num1 = status.GetValueOrDefault() == transactionStatus & status.HasValue ? 1 : 0;
                                        goto label_15;
                                    }
                                }
                            }
            ...
            get
            {
                if (HasEscrow)
                {
                    ...
                    if (CurrentState == DeviceManagerState.NONE && CurrentTransaction?.CurrentTransactionResult != null)
                    {
                        ...
                        else
                        {
                            ...
                            DeviceState deviceState = DeviceState.IDLE;
                            num3 = status1.GetValueOrDefault() == deviceState & status1.HasValue ? 1 : 0;
                        }
                        if (num3 == 0)
                        {
                            IControllerStatus currentDeviceStatus4 = CurrentDeviceStatus;
                            ...
                            else
                            {
                                ...
                                DeviceState deviceState = DeviceState.OK;
                                num4 = status1.GetValueOrDefault() == deviceState & status1.HasValue ? 1 : 0;
                            }
                            if (num4 == 0)
                                goto label_23;
                        }
                        if (HasEscrow)
                        {
                            ...
                            else
                            {
                                ...
                                EscrowStatus escrowStatus = EscrowStatus.IDLE_POS;
                                num5 = status2.GetValueOrDefault() == escrowStatus & status2.HasValue ? 1 : 0;
                            }
                            if (num5 == 0)
                                goto label_23;
                        }
                        ...
                        else
                        {
                            ...
                            DeviceTransactionStatus transactionStatus = DeviceTransactionStatus.ACTIVE;
                            num1 = status3.GetValueOrDefault() == transactionStatus & status3.HasValue ? 1 : 0;
                            goto label_24;
                        }
                    }
                label_23:
                ...
                if (CurrentTransaction?.CurrentTransactionResult != null && CurrentState != DeviceManagerState.TRANSACTION_ENDING)
                {
                    ...
                    else
                    {
                        ...
                        DeviceState deviceState = DeviceState.IDLE;
                        num7 = status.GetValueOrDefault() == deviceState & status.HasValue ? 1 : 0;
                    }
                    if (num7 == 0)
                    {
                        ...
                        else
                        {
                            ...
                            DeviceState deviceState = DeviceState.OK;
                            num8 = status.GetValueOrDefault() == deviceState & status.HasValue ? 1 : 0;
                        }
                        if (num8 == 0)
                            goto label_35;
                    }
                        ...
                        if (num2 != 0)
                        {
                            ...
                            if (num4 == 0)
                            {
                                ...
                                else
                                {
                                    ...
                                    DeviceTransactionStatus transactionStatus = DeviceTransactionStatus.ACTIVE;
                                    num1 = status1.GetValueOrDefault() == transactionStatus & status1.HasValue ? 1 : 0;
                                    goto label_22;
                                }
                            }
                            else
                        ...
                        if (num2 != 0)
                        {
                            ...
                            if (num4 == 0)
                            {
                                ...
                                else
                                {
                                    status1 = currentDeviceStatus5.Transaction?.Status;
                                    DeviceTransactionStatus transactionStatus = DeviceTransactionStatus.ACTIVE;
                                    num5 = status1.GetValueOrDefault() == transactionStatus & status1.HasValue ? 1 : 0;
                                }
                                if (num5 == 0)
                                    goto label_22;
                            }
                            ...
                            else
                            {
                                ...
                                num1 = statusResultStatus1.GetValueOrDefault() == statusResultStatus2 & statusResultStatus1.HasValue ? 1 : 0;
                                goto label_23;
                            }
                        }
                    }
            ...
            if (CurrentTransaction != null)
            {
                ...
                if ((currentDeviceStatus1 != null ? (currentDeviceStatus1.ControllerState == ControllerState.DROP ? 1 : 0) : 0) != 0)
                {
                    ...
                    else
                    {
                        ...
                        DeviceState deviceState = DeviceState.OK;
                        num2 = nullable1.GetValueOrDefault() == deviceState & nullable1.HasValue ? 1 : 0;
                    }
                    if (num2 != 0)
                    {
                        if (HasEscrow)
                        {
                            ...
                            else
                            {
                                ...
                                EscrowStatus escrowStatus = EscrowStatus.IDLE_POS;
                                num3 = nullable2.GetValueOrDefault() == escrowStatus & nullable2.HasValue ? 1 : 0;
                            }
                            if (num3 == 0)
                                goto label_19;
                        }
                        ...
                        if (num4 == 0)
                        {
                            ...
                            else
                            {
                                ...
                                DeviceTransactionStatus transactionStatus = DeviceTransactionStatus.NONE;
                                num1 = nullable3.GetValueOrDefault() == transactionStatus & nullable3.HasValue ? 1 : 0;
                                goto label_20;
                            }
                        }
                        else
            ...
            get
            {
                ...
                if ((num1 & num2) != 0)
                {
                    ...
                    else
                    {
                        ...
                        num4 = status.GetValueOrDefault() == transactionStatus & status.HasValue ? 1 : 0;
                    }
                    if (num4 != 0)
                    {
                        DeviceTransaction currentTransaction = CurrentTransaction;
                        ...
                        else
                        {
                            ...
                            num3 = statusResultStatus1.GetValueOrDefault() == statusResultStatus2 & statusResultStatus1.HasValue ? 1 : 0;
                            goto label_11;
                        }
                    }
                }
          ...
          StatusReport statusReport)
        {
            ...
            int result = int.TryParse(statusReport.Status.Sensors.Value, NumberStyles.HexNumber, new CultureInfo("en-US"), out result) ? result : 0;
        ...
        private DropStatusResult ProcessDropStatus(DropStatus dropStatus)
        {
            ...
            try
            {
                ...
                else
                {
                    ...
                    num1 = statusResultStatus2.GetValueOrDefault() == statusResultStatus3 & statusResultStatus2.HasValue ? 1 : 0;
                }
                int num2;
                if (num1 != 0)
                {
                    ...
                    num2 = totalCount1.GetValueOrDefault() == nullable.GetValueOrDefault() & totalCount1.HasValue == nullable.HasValue ? 1 : 0;
                }
                else
                    num2 = 0;
                if (num2 != 0)
            ...
            if (totalValue == valueOrDefault1 & nullable1.HasValue)
            {
                ...
                nullable1 = totalCount4.HasValue ? new long?(totalCount4.GetValueOrDefault()) : new long?();
        ...
        private DropResultResult ProcessDropResult(DropResult dropResult)
        {
            ...
            dropResultResult.TotalNumberOfNotes = (int)(dropResult?.Body?.TotalNumberOfNotes);
            dropResultResult.DroppedAmountCents = (long)(dropResult?.Body?.TranAmount);
            dropResultResult.TransactionNumber = dropResult?.Body?.TranCycle.ToString() + string.Empty;
          ...
          TransactionStatusResponse transactionStatusResponse)
        {
            ...
            statusResponseData.DispensedAmountCents = (long)(transactionStatusResponse?.Body?.DispensedAmount);
            ...
            {
                Body body = transactionStatusResponse.Body;
                denomination2 = body != null ? body.RequestedDispenseNotes.CreateDenomination() : null;
            }
            ...
            {
                Body body = transactionStatusResponse.Body;
                denomination3 = body != null ? body.TotalDroppedNotes.CreateDenomination() : null;
            }
        ...
        /*
            private List<(DateTime FileDate, FileInfo File)> GetCashAccSysLogFiles(
              ...
              DateTime EndDate)
            {
              List<string> list = ((IEnumerable<string>) Directory.GetFiles(this.CONTROLLER_LOG_DIRECTORY, "TRACE*.log", SearchOption.TopDirectoryOnly)).Reverse<string>().ToList<string>();
              ...
              DateTime startLogDate = source.Where<(DateTime, FileInfo)>((x => x..FileDate <= StartDate)).OrderByDescending<(DateTime, FileInfo), DateTime>((Func<(DateTime, FileInfo), DateTime>) (y => y.FileDate)).FirstOrDefault<(DateTime, FileInfo)>().Item1;
              DateTime endLogDate = source.Where<(DateTime, FileInfo)>((x => x.FileDate <= EndDate)).OrderByDescending<(DateTime, FileInfo), DateTime>((Func<(DateTime, FileInfo), DateTime>) (y => y.FileDate)).FirstOrDefault<(DateTime, FileInfo)>().Item1;
              return source.Where<(DateTime, FileInfo)>((x => x.FileDate >= startLogDate && x.FileDate <= endLogDate)).ToList<(DateTime, FileInfo)>();
            }
        */
        private bool IsEscrowJam(List<(DateTime FileDate, FileInfo File)> logFiles, DateTime startDate)
        {
            foreach (FileInfo fileInfo in logFiles.Select(x => x.File).ToList())
            {
                string str = File.ReadAllLines(fileInfo.FullName).Reverse().ToList().FirstOrDefault(x => x.Contains("JamOpenEscrowWait"));
        ...
        {
            if (DeviceManagerMode != DeviceManager.DeviceManagerMode.ESCROW_JAM && e.Status[4] == 'R')
                OnEscrowJamStartEvent(this, EventArgs.Empty);
            else if (DeviceManagerMode == DeviceManager.DeviceManagerMode.ESCROW_JAM && CurrentState != DeviceManagerState.ESCROWJAM_CLEAR_WAIT && e.Status[4] == 'I')
            {
                OnEscrowJamClearWaitEvent(this, EventArgs.Empty);
            }
            else
            {
                if (DeviceManagerMode != DeviceManager.DeviceManagerMode.ESCROW_JAM || CurrentState == DeviceManagerState.ESCROWJAM_END_REQUEST || e.Status[4] != '@')
                    return;
