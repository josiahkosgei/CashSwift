...
namespace CashAccSysDeviceManager
{
    public class CDCMessenger
    {
        ...
        private int _controllerSequenceNumber = 0;
        ...
        {
            Log = new CashSwiftLogger(Assembly.GetAssembly(typeof(CashAccSysDeviceManager)).GetName().Version.ToString(), "DeviceMessengerLog", null);
            ...
            client = new EventDrivenTCPClient(Log, _host, _port, AddressFamily.InterNetwork);
        ...
        private void dispTimer_Tick(object sender, EventArgs e)
        {
            try
            {
                if (OutGoingMessageQueue.Count <= 0)
                    return;
                if (OutGoingMessageQueue.Count <= 1)
                    ;
                string message = OutGoingMessageQueue.FirstOrDefault().Value;
                if (message != null)
                {
        ...
        private void OnConnectionEvent(object sender, AuthoriseResponse e)
        {
            if (OutGoingMessageQueue.Count > 0)
            {
                ...
                int leastIndex = OutGoingMessageQueue.FirstOrDefault().Key;
                OutGoingMessageQueue = new SortedList<int, string>(OutGoingMessageQueue.Where(x => !x.Value.Contains("MsgID=\"102\" MsgName=\"StatusReq\"")).Select(message => new KeyValuePair<int, string>(message.Key - leastIndex + seqNo + 11, message.Value.Replace(string.Format("SeqNo=\"{0}\"", message.Key), string.Format("SeqNo=\"{0}\"", message.Key - leastIndex + seqNo + 11)))).ToDictionary(newmessage => newmessage.Key, newmessage => newmessage.Value));
                SendMessage(seqNo + 10, SerializeToXML(new StatusRequest(seqNo + 10), StatusRequest.Serializer));
      ...
      "\n\n"
        ...
        internal void ShowDeviceController()
        {
            ...
            SendMessage(sequenceNumber, SerializeToXML(new ShowController(sequenceNumber), ShowController.Serializer));
        }

        public void Connect()
        {
            string xml = SerializeToXML(new AuthoriseRequest(_sequenceNumber, _clientID, _macAddress, _clientType), AuthoriseRequest.Serializer);
            ++_sequenceNumber;
            client.SendMessage(xml);
            Console.WriteLine(">OUT> " + xml);
        }

        internal void ResetDevice() => SendMessage(_sequenceNumber, SerializeToXML(new CancelCurrentProcess(_sequenceNumber), CancelCurrentProcess.Serializer));

        internal void GetStatus()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new StatusRequest(sequenceNumber), StatusRequest.Serializer));
        }

        internal void ClearOutgoingMessageQueue() => OutGoingMessageQueue.Clear();

        internal void SetCurrency(string currency) => SendMessage(_sequenceNumber, SerializeToXML(new SelectCurrency(_sequenceNumber, currency), SelectCurrency.Serializer));
        ...
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new DropRequest(sequenceNumber, Currency, UserID, dropType: DropMode.ToCashAccSysString(), reference: TransactionID, inputSubNumber: DropID, inputNumber: SessionID), DropRequest.Serializer));
        }

        internal void EndCount()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new DropEnd(sequenceNumber, "DONE"), DropEnd.Serializer));
        }

        internal void EscrowDrop()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new DropEnd(sequenceNumber, "ESCROW_DROP"), DropEnd.Serializer));
        }

        internal void EscrowReject()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new DropEnd(sequenceNumber, "ESCROW_REJECT"), DropEnd.Serializer));
        }

        internal void RequestPause()
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new DropPause(sequenceNumber), DropPause.Serializer));
        }
        ...
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new EndTransaction(sequenceNumber), EndTransaction.Serializer));
        }

        internal void EndCIT(string bagNumber)
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new OpenBagRequest(sequenceNumber, bagNumber), OpenBagRequest.Serializer));
        }

        internal void StartCIT(string sealNumber)
        {
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new CloseBagRequest(sequenceNumber, sealNumber, CloseBagRequestAction.SUMMARY_REPORT), CloseBagRequest.Serializer));
        }

        public void GetTransactionStatus()
        {
            IEnumerable<bool> source = OutGoingMessageQueue.Select(x =>
            {
                string str = x.Value;
            ...
            if (num1 == 0)
                return;
            int sequenceNumber = _sequenceNumber;
            SendMessage(sequenceNumber, SerializeToXML(new TransactionStatusRequest(sequenceNumber), TransactionStatusRequest.Serializer));
        }
    }
}
