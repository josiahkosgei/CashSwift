...
// Type: CashAccSysDeviceManager.CashAccSysSerialFix
...
namespace CashAccSysDeviceManager
{
    public class CashAccSysSerialFix : IDisposable, INotifyPropertyChanged
    {
        ...
        private bool quitSendingSerial = false;
        private bool dE50CassetteFull = false;
        ...
        private bool AllowClearEscrowJam = false;
        private string fromDE50 = null;
        private CashSwiftLogger Log = new CashSwiftLogger(Assembly.GetExecutingAssembly().GetName().Version.ToString(), "SerialFix", null);
        private CashSwiftLogger DropLog = new CashSwiftLogger(Assembly.GetExecutingAssembly().GetName().Version.ToString(), nameof(DropLog), null);
        private bool _canDrop = false;
        private bool _isStorageJam = false;
        private string lastPartialFrame = null;
            ...
            while (!quitSendingSerial)
            {
             ...
             lastPartialFrame
                    ...
                    else
                    {
                        ...
                        if (lastPartialFrame1 != null && lastPartialFrame1.Contains("\u0002"))
                            lastPartialFrame = new string(lastPartialFrame.Skip(lastPartialFrame.IndexOf("\u0002")).ToArray());
                        string lastPartialFrame2 = lastPartialFrame;
                        if (lastPartialFrame2 != null && lastPartialFrame2.Contains("\u0003"))
                        {
               ...
               FromDE50
                        ...
                        if (FromDE50.Length == 118)
                        {
                            Log.Info(GetType().Name, "process message", "changed SR2 to BatchEnd", "RX: {0}", new object[1]
                            {
                 FromDE50
                            });
                            byte[] bytes = Encoding.ASCII.GetBytes(FromDE50.ToCharArray(), 0, FromDE50.Count());
                            bytes[5] &= 251;
                            FromDE50 = Encoding.ASCII.GetString(bytes);
                        }
