                ...
                while (true)
                {
                    ...
                    if (!string.IsNullOrWhiteSpace(reader.Name))
                    {
                        try
                        {
                            ...
                            if (doc.Root.Attribute((XName)"MsgID").Value != "10")
                                SendMessage("<?xml version=\"1.0\" encoding=\"UTF-8\"?><CCP MsgID=\"10\" MsgName=\"ACK\" SeqNo=\"" + doc.Root.Attribute((XName)"SeqNo").Value + "\"><body /></CCP>");
